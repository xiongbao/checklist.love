...
func GenerateFeed(c *gin.Context) {
	...
	for _, disdiscussion := range discussions.Nodes {
		...
		feed.Items = append(feed.Items, &feeds.Item{
			...
			Link:        &feeds.Link{Href: fmt.Sprintf("%s/item/%d", config.Website.Host, disdiscussion.Number)},
		})
	}
...
func main() {
	...
	r.GET("/item/:id", cache.CacheByRequestURI(memoryCache, 1*time.Hour), FetchPost)
